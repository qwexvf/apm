...
description: How ccpm resolves semver ranges, commit SHAs, and branch refs.
sidebar:
  label: Version constraints
  order: 30
---

ccpm supports four kinds of version specifiers. The kind is auto-detected from the string format.
...
The constraint is saved in `ccpm.toml`; the resolved exact version is saved in `ccpm.lock`.
...
> [!WARNING]
> Branch refs are resolved to a commit SHA at install time. The SHA is stored in `ccpm.lock`, so subsequent `ccpm install` calls are still reproducible. But `ccpm update` will move to the new HEAD — use a SHA if you need a harder pin.
...
When resolving, ccpm detects constraint kind in this order:
